...
function writeServerConfig(config) {
  ...
  // Re-point the Cloudflare Access mirror at the new host. Without this the
  // sidecar keeps publishing the previous server's cookie after a switch, which
  // reads as "login required" against a host that never issued it.
  void trackCloudflareAccessServer(merged?.serverUrl || "").catch(() => {});
  return merged;
}
...
app.whenReady().then(async () => {
  ...
  // Point the Cloudflare Access mirror at the configured server: seeds the
  // sidecar's cookie copy from any still-valid prior login (so a cold start
  // authenticates without a login window) and subscribes to the cookie jar so
  // later CF rotations keep it in sync.
  try {
    await trackCloudflareAccessServer(String(getStoredServerConfig?.()?.serverUrl || "").trim());
  } catch (cfSeedError) {
