...
export function registerAllIpcHandlers() {
  ...
  safeLocalStoreInvoke("local-store-record-sale", (store, payload) => store.recordSale(payload));
  safeLocalStoreInvoke("local-store-list-sales", (store, userId) => store.listSales(userId));
  safeLocalStoreInvoke("local-store-list-sale-allocations", (store, saleId) => store.listSaleAllocations(saleId));
  safeLocalStoreInvoke("local-store-list-consumed-quantities", (store, userId) => store.listConsumedQuantities(userId));
  safeLocalStoreInvoke("local-store-delete-sale", (store, id, userId) => store.deleteSale(id, userId));
  safeLocalStoreInvoke("local-store-get-sale", (store, id) => store.getSale(id));
  safeLocalStoreInvoke("local-store-import-sales", (store, rows, userId) => store.importSales(rows, userId));
  safeLocalStoreInvoke("local-store-delete-sale-silent", (store, id) => store.deleteSaleSilent(id));
  ...
  safeLocalStoreInvoke("local-store-import-wallet-events", (store, rows, userId) => store.importWalletEvents(rows, userId));
  safeLocalStoreInvoke("local-store-delete-wallet-event-silent", (store, id) => store.deleteWalletEventSilent(id));
  safeLocalStoreInvoke("local-store-mark-wallet-event-pushed", (store, id, at) => store.markWalletEventPushed(id, at));
  safeLocalStoreInvoke("local-store-enqueue-dirty-wallet", (store, userId) => store.enqueueDirtyWalletOperations(userId));
  ...
  safeLocalStoreInvoke("local-store-list-operations", (store, userId, limit) => store.listOperations(userId, { limit }));
