...
- Frontend color gradients must use the shared avatar-derived Steam palette variables (`--steam-shell-color-a` ... `--steam-shell-color-d`), with static values allowed only as fallback when avatar data is unavailable. The four variables are declared with those fallbacks in `:root`; the runtime override is an inline style on the root element and therefore wins.
- **Everything that is not a Steam gradient runs on the semantic token scale** in `apps/web/src/index.css` (`@theme inline` + `:root`/`.dark`): surfaces (`background`, `foreground`, `card`, `popover`, `muted`, `accent`, `border`, `input`, `ring`) and status (`success`, `warning`, `info`, `destructive`, each with a `-foreground`). There are no hardcoded Tailwind palette classes left in the repo. A token flips per theme by itself, so a `dark:` variant of the same token is redundant — that redundancy is what previously forced a block of `!important` light-mode repairs for `.steam-journey-card`, now deleted along with its cause.
- **Each status colour exists twice, because it has two incompatible jobs.** `--<token>` is the *on-surface* tone: it is read against the card, so it flips per theme (light ≈ `oklch(0.52 …)`, dark ≈ `0.72–0.80`) and serves `text-<token>` plus the `bg-<token>/12` tint idiom, whose small text needs 4.5:1. `--<token>-solid` with `--<token>-foreground` is the *fill* pair for buttons, badges and status dots; it is read against its own label, never against the card, and therefore does **not** flip — a destructive button is the same red in both themes. Collapsing the two roles into one value is what previously made dark-mode danger buttons pastel with dark labels. Both pairings are measured in both themes; changing a status colour means re-checking them, not eyeballing it.
- UI primitives live in `packages/shared/src/components/ui/` (shadcn idiom: `cva` variants + `cn`, Radix underneath). `BaseModal` is a Radix Dialog wrapper that keeps the project's own bottom-sheet-on-mobile and fullscreen layouts; because callers scope modals with `md:hidden`, its overlay and content share one positioned wrapper instead of being portal siblings.
