...
- root `main.js` and root `preload.js` still exist, while active Electron entry is `apps/desktop/main.js`. Nothing references them — `package.json`'s `main` points at `apps/desktop/main.js`.
- `src.old/` is gone; the migration remainder no longer exists.
...
- **Desktop Steam login (browser handoff)**: the desktop app opens the Steam OpenID URL in the *system browser* (`shell.openExternal`), because that is where the user's Steam session already lives. It cannot observe that browser's callback, so `GET /api/v1/auth/steam/login` accepts a `handoffKey` (SHA-256 of a client-generated secret), the callback parks its outcome in `auth_login_handoffs`, and the app claims it via `GET /api/v1/auth/steam/result?state=&claim=` — single-use, 15-minute TTL (the CSRF state gets the same TTL, so the callback cannot outlive the pickup), public route guarded by the claim secret rather than a session that does not exist yet. The handoff table's `expires_at` must stay a **DATETIME**: as the first `TIMESTAMP NOT NULL` column without a DEFAULT it would inherit `ON UPDATE CURRENT_TIMESTAMP`, and storing the finished login would then expire the very row that stores it. The pickup distinguishes `pending`, `ready`, `missing` (404), an unreadable payload (500 `HANDOFF_PAYLOAD_UNREADABLE`, carrying only the payload's byte count and the JSON decoder's verdict — never any of its content) and a server without the store (503 `HANDOFF_UNAVAILABLE`), and it only consumes the row once the payload has decoded. The parked login also covers the *failure* case, so a rejected login surfaces immediately instead of after the poll timeout. The callback renders a "close this tab" page for a parked login and keeps the old `cs-portfolio://…#token=` redirect for the in-app window flow, which stays as the fallback whenever the server response carries no `handoff: "poll"`.
- **Sidecar auth handoff files**: the desktop OpenID flow parks its state and its finished result — the latter containing a session token — in `csportfolio_desktop_auth_state.json` / `csportfolio_desktop_auth_result.json` under `DESKTOP_STATE_DIR` (default: system temp), each valid for 5 minutes. Both read paths in `DesktopSteamAuthController` drop expired entries and rewrite the file, because the write-time pruning they had before never ran on machines that always log in through the server flow, leaving a consumed token on disk indefinitely.
- **Session token formats**: server (`SteamAuthController`) and desktop sidecar (`DesktopSteamAuthController`) use mutually incompatible encryption (raw key + 16-byte IV vs. `sha256`-derived machine-local key + 12-byte IV). Desktop login therefore uses the server flow ("Variante C") whenever a server is configured, and the sidecar login only when none is — a failed server login raises its own error instead of falling back to a local-only token that cannot sync. A `401` with `AUTH_REQUIRED`/`INVALID_SESSION`/`MISSING_TOKEN` makes `desktopSync.js` clear the session once and report through `sessionHealthBus.js`, which surfaces the state as a warning badge on the avatar (`UserMenu`) and as a persisted `session_health` notification.
- **The sync endpoint fallback must not mistake the SPA for the API.** `fetchSyncEndpointWithFallback` (`desktopSync.js`) walks the candidate URLs from `buildSyncEndpointCandidates`; the deployed server serves the SPA for every path it does not route to the API, so `/index.php/api/v1/sync/pull` answers `200 text/html`. Accepting that `ok` response as the result threw away the JSON `401` the `/api/index.php` candidate had already produced — the sync failed with "HTML instead of JSON" on every run, `handleDeadSessionResponse` never saw the `AUTH_REQUIRED` code, and an expired session token kept 401-ing forever while the app looked signed in. An HTML response is therefore never a success (only a last-resort return value), and a JSON `401`/`403` is authoritative and ends the candidate search.
...
purchase rows** — the remaining holding is derived from `quantity` minus what
...
| `docs/video/shotlist.md` | IN PROGRESS | CURRENT | Showcase-video shot list; the vault beats are shot (commit `2eda1e4`). |
...
| `docs/fee-settings-plan.md` | IN PROGRESS | CURRENT | Matches versioned `user_fee_settings` + current API wiring. Its per-investment `funding_mode` model is superseded by `docs/wallet-cost-basis-plan.md`. |
| `docs/wallet-cost-basis-plan.md` | PLANNED | CURRENT | Why per-position `funding_mode` cannot work and the blended wallet-factor model that replaces it. |
