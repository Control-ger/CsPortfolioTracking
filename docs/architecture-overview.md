...
- Server sync item resolution (`SyncEntityService::resolveItemIdForSync`) is relational: trust a valid payload `itemId` (FK to `items`), else resolve by `market_hash_name` (item natural key, UNIQUE). Image URL is an attribute, not a key — matched only by exact URL. The removed fuzzy image-token `LIKE` fallback + "canonical-by-image" redirect cross-linked different skins sharing image-token prefixes (Dreams & Nightmares Case → Stiletto knife `item_id`).
