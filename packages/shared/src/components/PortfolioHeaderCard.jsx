...
const resolveSyncHealth = (oldestAgeSeconds, liveItemsCount) => {
  if (!Number.isFinite(liveItemsCount) || liveItemsCount <= 0) {
    return {
      ...
      iconClass: "rounded-full border border-border/35 bg-muted/12 p-2 text-muted-foreground",
      icon: "clock",
    };
  }
  ...
  if (oldestAgeSeconds <= 90 * 60) {
    return {
      label: "im plan",
      tone: "text-success",
      iconClass: "rounded-full border border-success/30 bg-success/12 p-2 text-success",
      icon: "clock",
    };
  }
  ...
  if (oldestAgeSeconds <= 3 * 60 * 60) {
    return {
      label: "verzoegert",
      tone: "text-warning",
      iconClass: "rounded-full border border-warning/30 bg-warning/12 p-2 text-warning",
      icon: "refresh",
    };
  }
  ...
  return {
    ...
    iconClass: "rounded-full border border-destructive/30 bg-destructive/12 p-2 text-destructive",
    icon: "refresh",
  };
};
...
export const PortfolioHeaderCard = ({
  ...
  const trendColor = effectiveIsPositive ? "text-success" : "text-destructive";
