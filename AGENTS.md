...
- Manual matching: `createManualSteamCsfloatMatch` (localStore `sync.js`, IPC `local-store-create-manual-steam-csfloat-match`) lets the user link a Steam asset to a CSFloat/SkinBaron position the scorer never paired. It upserts on `UNIQUE(user_id, steam_asset_id, csfloat_investment_id)` with `status='manual_confirmed'`, `reason='manual_link'` and score 0, then runs the same `applySteamCsfloatMatchLink` as the automatic path — so a manual link promotes an existing suggested/rejected row instead of duplicating it. Both candidate lists in the UI exclude positions that are already part of a resolved match.
...
- Portfolio groups carry a `color` token (`success|info|warn|danger|muted`, never a raw colour) whitelisted in **two** places that must stay in lockstep: `PORTFOLIO_GROUP_COLORS` (`packages/shared/src/lib/portfolioGroups.js`) and `GROUP_COLORS` (`UserPortfolioGroupsRepository.php`); both normalise unknown values to `success`. Because the settings PUT response is adopted as authoritative, a backend predating that whitelist silently drops `color` and would reset it on every save — `preservePortfolioGroupColors` re-applies the local colour wherever the echo omits it (save path *and* load/merge path). Remove that guard only once the deployed server echoes `color`.
...
- App-update availability surfaces **only** in the in-app notification bell (persisted `category=app_update` notifications from `apps/desktop/main/updater.js`) and the Settings "Über die App" card ("Nach Updates suchen" + live status). Do **not** reintroduce a floating in-app update toast or native OS toast. Auto-check is ~15s after launch + every 10 min (check-only; download/install stay user-confirmed). Because the bell is the only channel, the updater must write under the **active Steam scope** (`steam-<steamId>`), not numeric `1`: `setActiveNotificationUserId` (`updater.js`) records the scope the renderer polls `local-store-list-notifications` with, falling back to `resolveActiveLocalUserId()` in `localStore/sync.js`. A legacy-scoped write is invisible — same rule as `session_health`. The notification write needs the **live store instance** (`setLocalStoreRefs(() => localStore, …)`); `getLocalStore` is the async module loader and returns a Promise, which the store check rejects — every update notification then disappears without a log. `app-updater-status` is fire-and-forget, so the last status is cached and replayable via `app-updater-last-status` / `updater.getLastStatus()`, pulled once per renderer mount (live push wins on a race). Download progress is the one bell entry that is **not** persisted: `DesktopSidebarRail` renders it as an ephemeral row built from the live `state="downloading"` push (`buildDownloadProgressEntry`), because progress fires many times per second — persisting it would hammer SQLite and orphan a row if the app dies mid-download. It vanishes on any other state and forces an immediate reload so the persisted "Update bereit" row takes over. **Not every install can update itself** (AppImage started without `APPIMAGE`, Snap, unpacked build): `resolveSelfUpdateSupport()` asks electron-updater's `isUpdaterActive()` first, and when it says no the updater switches to a GitHub-release-feed check that emits `state="manual"` plus the releases URL instead of a download. Every dead end (manual mode, updater error, failed download) must carry `url` so the UI can offer the releases page — `runAppUpdateAction()` in `packages/shared/src/lib/appUpdateActions.js` is the single shared implementation for all three surfaces (rail bell, CS updates page, portfolio dropdown); do not fork it again.
- Window buttons in the frameless titlebar are **detected per desktop**, never hardcoded per platform: `apps/desktop/main/window-controls-theme.js` (IPC `window-controls-theme`) resolves button order/side from gsettings `button-layout` and the artwork from the active GTK theme's `metacity-1/titlebuttons/` assets, then the icon theme's `window-*` icons (symbolic ones are painted as a CSS mask), then the built-in Windows/macOS presets. Users can force a preset via Settings → Fenster-Buttons (localStorage, `packages/shared/src/lib/windowControls.js`).
...
- Startup order is **Steam login first, vault gate second**: the vault unlock/setup screen in `App.jsx` renders only when a desktop Steam session exists; otherwise `PortfolioPage`'s `SteamLoginPrompt` takes over. Do not gate the app on the vault before the session check — after a logout the vault is still configured but locked, which would demand the app password for an unconnected account.

### Auth/User Scope
- **Cloudflare Access strips the `Authorization` header before it reaches the origin.** Every direct renderer→server call must therefore send the session token in **both** `Authorization: Bearer <token>` **and** `X-Auth-Token: <token>`. Server-side both are accepted (`RequestAuthenticator::extractSessionToken`, and the `/api/v1/auth/session/validate` route reads `HTTP_AUTHORIZATION ?? HTTP_X_AUTH_TOKEN`). Symptom when only `Authorization` is sent: the server answers `MISSING_TOKEN` / `AUTH_REQUIRED` although the client holds a perfectly valid token — the header simply never arrives. Verified by measurement: a curl inside the container reaches PHP with the header intact, the same request through the tunnel does not. `X-Auth-Token` must also stay listed in the server's `Access-Control-Allow-Headers`, otherwise the CORS preflight rejects it.
- **Two incompatible session token formats exist.** `SteamAuthController` (server) signs with the raw `ENCRYPTION_KEY`, a 16-byte IV and base64 ciphertext; `DesktopSteamAuthController` (sidecar) signs with `hash('sha256', key, true)`, a 12-byte IV and `OPENSSL_RAW_DATA`, using a **machine-local** key from Electron `safeStorage`. A sidecar token can therefore never be validated by the server. `initiateDesktopSteamLogin` tries the server login (Variante C) first and only falls back to the sidecar login; that fallback token is local-only and **cannot sync**. Never route a sidecar-issued token to server endpoints.
- **A rejected session must be cleared, not retried forever.** `desktopSync.js` treats a `401` carrying `AUTH_REQUIRED`, `INVALID_SESSION` or `MISSING_TOKEN` as a definitively dead token: it calls `logout()` once and reports via `sessionHealthBus.js`, so the next login issues a fresh server token. `USER_SCOPE_FORBIDDEN` (403) is excluded — that is a scope mismatch on a valid session. Without this a single failed login at setup time left the app permanently 401-ing while looking logged in (the avatar renders from the cached local user object).
- **Broken sync sessions must be visible.** `sessionHealthBus.js` surfaces both the sidecar fallback and a server-rejected session through two channels: a warning badge on the avatar in `UserMenu` (plus an explanatory row inside the opened menu), and a persisted `category=session_health` entry in the notification centre. A `console.error` alone is not sufficient — it is invisible to users and turns a one-off login failure into an undiagnosable permanent outage. The notification is written under `resolveDesktopLocalUserId(user, 1)` (`steam-<steamId>`), the same scope the notification centre reads with; writing it under numeric `1` would file it where nothing looks.
...
- **Identity is derived once, server-side, and never from a request header.** `RequestAuthenticator` decrypts the session token exactly once per request and produces a `RequestIdentity` (userId, steamId, IP) that the auth gate, the rate limiter and `RequestUserScopeResolver` all share. `X-User-Id` / `X-Steam-Id` / `userId` body fields are only ever *compared* against it. Never reintroduce a path that treats them as identity: the controllers used to carry a `resolveUserId()` fallback that read `x-user-id` straight out of the request whenever the scope resolver was absent — the resolver is now a required constructor dependency and that fallback is gone.
- **The auth gate is deny-by-default** (`obs_apply_auth_gate`, allowlist in `RouteAccessPolicy`). Only Steam login/callback, `auth/session/validate`, `push/public-key`, `exchange-rate` and `cs-updates` are public; a new route is private until someone consciously lists it. Before this, a request with no credential at all reached `RequestUserScopeResolver`'s `return 1` and was served the default account's portfolio, watchlist and settings — Cloudflare Access was the only thing in front of it. Rollout is flag-gated: with `AUTH_REQUIRED_ENFORCED=false` (default) the gate only logs `security.auth.request_denied`; flip it to `true` **after** verifying a real desktop run against the log, because the desktop sidecar's upstream proxy paths must all be carrying a session token.
- **`SteamAuthController::validateSession()` must stay free of outbound calls** — it is on every authenticated request. Profile backfill from Steam lives in `validateSessionWithProfile()`, used only by `GET /api/v1/auth/session/validate`.
- **Rate limiting is two-staged** (`RateLimitPolicy`, `RateLimitGuard`): `STAGE_EDGE` runs pre-DB keyed on the connection IP (auth endpoints fail-closed, plus a global per-IP ceiling so a flood never reaches MySQL); `STAGE_SESSION` runs after token validation keyed on the hashed session subject and holds the per-route budgets. Bucket keys are a peppered SHA-256 of the session subject (or IP) — internal only, never returned to the client, so the store and the `security.rate_limit.blocked` events hold no plaintext Steam IDs or IPs. Rules match via `RoutePattern` (`{id}`, `{key}`, `*`), most specific wins. Do not key a bucket on anything the caller controls: that was the original bug — rotating `X-User-Id` produced an unlimited number of fresh budgets.
- Proxy headers (`CF-Connecting-IP`, `X-Forwarded-For`) are honoured only when `TRUST_PROXY_HEADERS=true`; otherwise `ClientIpResolver` falls back to `REMOTE_ADDR`. Keep it true only while the origin is exclusively reachable through the Cloudflare tunnel.
- **Session tokens carry a `jti` and are revocable via `user_sessions`.** The token itself stays stateless (AES-256-GCM, 30 days); the table only answers "may this token still be used". Two invariants that must not be softened: an **unknown `jti` is treated as revoked** (otherwise a pruned row silently resurrects a token and logout means nothing), and the row is **written before the token is returned** (a token whose registration failed must never be live). The single deliberate fail-open is a registry *read* error — a DB blip must not sign everyone out. `POST /api/v1/auth/logout` is public on purpose: a client logging out often holds a token the gate already rejects and still needs to clean up. Legacy tokens without `jti` are accepted only while `SESSION_LEGACY_TOKENS_ALLOWED=true`; flip it to `false` once 30 days (the token lifetime) have passed since rollout.
- **The rate limiter is a weighted sliding window, not a fixed one.** A fixed window allows a double-rate burst across the boundary (full budget at the end of window N, again at the start of N+1) — which is worst on the login endpoint. Do not "simplify" it back to a plain counter. Counter storage is APCu when available, else a locked JSON file; probe with `apcu_enabled()` (SAPI-aware), never `ini_get('apc.enabled')`, or CLI ends up with a store that always fails instead of falling back.
- Desktop sidecar forwards `Authorization`/`X-Auth-Token` to server upstreams so scope checks remain effective.
- The upstream server (incl. `/api`) sits behind a Cloudflare Zero Trust tunnel; the sidecar's upstream curl proxy authenticates by forwarding the renderer's per-user CF Access cookie. The Electron header bridge injects it as `X-Upstream-Cf-Cookie` on every renderer→sidecar request and `backend/desktop/index.php` promotes it into `UPSTREAM_COOKIE_HEADER` per request. No shared service token — each user uses their own CF identity.
- **All Cloudflare Access logic lives in exactly two modules** — `apps/desktop/main/cloudflare-access.js` (main) and `packages/shared/src/lib/cloudflareAccess.js` (renderer). Do not add a local challenge detector or a direct `cloudflareAccess.login()` call anywhere else; three divergent copies is what produced the popup loop.
- **The sidecar's cookie copy tracks the cookie jar, it is not a snapshot.** `defaultSession` is the single source of truth; the main module subscribes to `cookies.on("changed")` and republishes. A snapshot taken once at startup goes stale the first time Cloudflare rotates `CF_Session`, and every proxied read then reports a challenge for a session that is perfectly valid.
- **A challenge escalates in two steps: silent refresh, then login window.** `ensureCloudflareAccessSession(url)` re-reads the jar and returns without a window when a valid Access identity exists; only `{force: true}` opens one. Opening a window first is destructive — it clears the Access cookies before loading, so a false-positive challenge logs the user out of Cloudflare, and the silent SSO that follows makes the window flash open and shut.
- Challenge detection is status-gated (`401/403` + CF markers, or a `/cdn-cgi/access/` URL). Never classify by `text/html` + `server: cloudflare` alone — that matches any HTML error page the edge serves.
- Cloudflare cookies are never persisted to `desktop-session.json`. That file is the Steam session store and `session-store set` overwrites it wholesale, so anything else written there is lost on the next login.
...
- **Year Wrapped (`/wrapped`, `YearWrappedPage`) is desktop-only and reads raw local rows.** It calls `localStore.listInvestments()` / `listWatchlist()` directly: both clustering paths (`clusterDesktopInvestments`, server-side `aggregateInvestmentsByName`) collapse rows by item name and drop the `purchasedAt` that every buy statistic needs — and the web API returns exactly those aggregated, dateless rows, which is why there is no web variant. `usePortfolio()` supplies only `portfolioHistory` + `enrichedInvestments`. Pure stats live in `packages/shared/src/lib/yearWrapped.js` (date fallback `purchasedAt || importedAt || createdAt`; per-block `available` flags so empty slides are dropped). The route is intentionally absent from `DesktopSidebarRail`, the two page-local rail copies (`PortfolioPage`, `CsUpdatesPage`) and `BottomNavigation`; the only in-app entry is the seasonal dashboard banner (`resolveWrappedSeason`: 15-31 Dec = ending year, January = year just ended), dismissed via the year-scoped key `year-wrapped:dismissed:<year>`. Realized P&L is out of scope — no exit model exists. **Excluded positions must be filtered** (`isExcludedRow`) before any purchase statistic: raw local rows still contain them, and counting them inflates every spend figure (observed: a year whose 12 rows were all excluded reported 100 pieces and handed "most expensive purchase" to an excluded AWP). A year with only excluded rows has no data and renders a dedicated empty state.
- **UI sounds are global** (`packages/shared/src/lib/uiSounds.js`): Web Audio-synthesized tones, no audio assets (Howler/`use-sound` need sample files; Tone.js is ~200 kB for a few blips), CSP-safe. Enabled flag + volume live in `localStorage` like the theme — deliberately not in `portfolioPreferences`, which would force three normalization sites to stay in sync for a mute toggle. `SoundSettingsSection` and the Wrapped header both drive the module and mirror it via `subscribeUiSounds`. Playback is best-effort; `primeUiSounds()` must run from a real user gesture or the autoplay policy keeps the AudioContext suspended. `useCountUp` needs its `setTimeout` safety net: `requestAnimationFrame` is dead in a hidden/occluded window, and without it counters display `0` — a wrong number, not just a missing animation.
- App-wide click feedback is **one delegated listener** (`useGlobalUiSounds`, mounted in `App.jsx`), not per-component calls — three rail copies plus modals and settings would drift. Capture-phase `pointerdown` so `stopPropagation` handlers are still seen, and it doubles as the AudioContext-unlocking gesture. Controls with their own semantic sound opt out via `data-no-sound` (honored on ancestors too), else they play the generic click on top of their own tone.
- **Never fill chart marks with `--steam-shell-color-*`**: those carry a baked 0.11-0.20 alpha (background washes) and render marks at ~15% opacity — the reported "empty donut". Use `buildChartPaletteVars` (`packages/shared/src/lib/steamChartPalette.js`) for opaque `--wrapped-chart-a…d` siblings that keep the avatar hue.
...
- `docker-entrypoint.sh` fixes log ownership at container start, before any writer runs. Apache/PHP (`www-data`) and the supervisord crons (root) append to the same `app.log`; whoever creates it first owns it, and a root-created file silently locks web requests out. The build-time `chown` cannot cover this because a bind-mounted host directory overlays the image layer.
- The compose file bind-mounts `${PROJECT_ROOT_PATH}/logs` so `app.log` outlives the container replacement of every Watchtower update; the host directory must be owned by UID 33 or the mount overlays the image's ownership and re-breaks web-request logging. `backend/.htaccess` ships in the image (bare `/api/v1/...` → front controller); do not move it into a host bind mount, since `create_host_path: true` turns a missing source file into a *directory* that silently disables the rule.
- **The container log directory must be writable by `www-data`.** The Dockerfile creates `/var/www/html/logs` and chowns it; every `COPY` lands root-owned, and Apache serves as `www-data`. Without it `FileSink`'s silenced `@file_put_contents` fails for **every web request** while the root-owned CLI crons keep appending to the same file — the log looks healthy but contains no request-scoped events at all. That is how zero `security.auth.request_denied` entries were mistaken for "no unauthenticated traffic". `FileSink` now reports an unwritable log once per process via `error_log`. Note that `php://stderr` from mod_php does **not** reach `docker logs` in this image, so `app.log` is the only usable source.
