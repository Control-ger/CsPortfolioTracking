...
- Server item resolution during sync (`SyncEntityService::resolveItemIdForSync`) is strictly relational: a valid payload `itemId` (FK to `items`) is trusted as-is; otherwise the item is resolved by `market_hash_name` (the item natural key, UNIQUE in `items`). **Image URL is an attribute, never a key** — `findItemIdByImageUrl` does an exact-URL match only. A previous fuzzy Steam-image-token `LIKE` fallback (and an "canonicalize a valid itemId by image" redirect) cross-linked distinct skins that share long image-token prefixes (observed: a Dreams & Nightmares Case position bound to a Stiletto knife `item_id`); both were removed.
