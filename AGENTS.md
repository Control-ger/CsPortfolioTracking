...
- **Broken sync sessions must be visible.** `SessionHealthBanner` (rendered in `App.jsx`) surfaces both the sidecar fallback and a server-rejected session. A `console.error` alone is not sufficient — it is invisible to users and turns a one-off login failure into an undiagnosable permanent outage.
