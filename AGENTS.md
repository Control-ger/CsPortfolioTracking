...
- Steam↔external matching covers **CSFloat and SkinBaron** rows as candidates (platform/id-prefix filter in `syncSteamInventory`); resolved matches exclude the external duplicate and copy its buy price onto the price-less Steam row. Match rows live in `steam_csfloat_matches` regardless of the external platform.
